...
GossipApp::StartApplication (void)
{
  ...
  Packet pack1(1024);
  Ptr<Packet> p = &pack1;
  m_socket_send[0]->Send(p);
  std::cout<<"a packet was sent to node 0 when node "<<m_node_id<<" begins"<<std::endl;
  ...
  Simulator::Schedule(Seconds(20.), &GossipApp::ConsensProcess, this); // TODO debug
...
void GossipApp::InitializeEpoch()
{
  for(int i=0; i<NODE_NUMBER; i++)
  {
